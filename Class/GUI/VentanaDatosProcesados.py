...
class VentanaDatosProcesados(Ventana):
    def __init__(self, root, window_manager, numero_lote, productos, legajo):
        ...
        self.legajo = legajo  # Inicializar antes de llamar a super().__init__()
    ...
    def crear_widgets(self):
        ...
        label_lote = tk.Label(frame, text=f"Legajo: {self.legajo}", font=("Arial", 12))
        ...
        btn_imprimir = tk.Button(frame, text="Exportar a PDF", command=lambda: aux.exportar_a_pdf(self.numero_lote, self.productos, self.legajo), width=20, height=3, font=("Arial", 16))
